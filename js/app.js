/* =============================================
   NEXMARK — App Logic
   localStorage-based bookmark + notes + cloud manager
   ============================================= */
...
// ── Storage helpers ─────────────────────────────
const KEYS = { bm: 'nx_bookmarks', notes: 'nx_notes', cloud: 'nx_cloud', prefs: 'nx_prefs' };
...
function init() {
  ...
  // Search
  document.getElementById('bm-search').addEventListener('input', debounce(renderBookmarks, 200));
...
function saveBookmark() {
  ...
  catch { showToast('Enter a valid URL', 'error'); return; }
  ...
  if (STATE.editingBookmarkId) {
    ...
    if (idx >= 0) {
      STATE.bookmarks[idx] = { ...STATE.bookmarks[idx], url, title: autoTitle, desc, tags, favicon: autoFavicon, updatedAt: Date.now() };
    }
    showToast('Bookmark updated ✓', 'success');
  } else {
    const bm = { id: uid(), url, title: autoTitle, desc, tags, favicon: autoFavicon, createdAt: Date.now() };
    STATE.bookmarks.unshift(bm);
    showToast('Bookmark saved ✓', 'success');
  }

  save(KEYS.bm, STATE.bookmarks);
  updateTagFilter();
  renderBookmarks();
  updateStats();
  closeModal('bm-modal');
...
function openNote(id) {
  ...
  editor.innerHTML = `
    <div class="note-editor-active">
      ...
      <div class="note-toolbar">
        ...
        <button class="note-tool-btn del-btn" onclick="deleteNote('${id}')" style="margin-left:auto;color:var(--red)">Delete</button>
      </div>
      <textarea class="note-content-input" id="note-content-inp" placeholder="Start writing…">${escHtml(note.content)}</textarea>
      <div class="note-footer">
        <span class="note-wordcount" id="note-wc">0 words</span>
        <span class="note-wordcount">Auto-saved</span>
      </div>
    </div>
  `;
  ...
  titleInp.addEventListener('input', () => {
    scheduleNoteSave(id, titleInp.value, contentInp.value);
  });

  contentInp.addEventListener('input', () => {
    updateWordCount(contentInp.value);
    scheduleNoteSave(id, titleInp.value, contentInp.value);
  });

  // Tab key in textarea
  contentInp.addEventListener('keydown', e => {
    if (e.key === 'Tab') {
      e.preventDefault();
      const s = contentInp.selectionStart;
      const v = contentInp.value;
      contentInp.value = v.slice(0, s) + '  ' + v.slice(s);
      contentInp.selectionStart = contentInp.selectionEnd = s + 2;
    }
  });
}
...
function scheduleNoteSave(id, title, content) {
  ...
  STATE.noteAutoSaveTimer = setTimeout(() => {
    ...
    if (idx >= 0) {
      STATE.notes[idx].title   = title || 'Untitled Note';
      STATE.notes[idx].content = content;
      STATE.notes[idx].updatedAt = Date.now();
      save(KEYS.notes, STATE.notes);
      // Update sidebar list title
      const item = document.querySelector(`[data-note-id="${id}"] .note-item-title`);
      if (item) item.textContent = title || 'Untitled Note';
      const prev = document.querySelector(`[data-note-id="${id}"] .note-item-preview`);
      if (prev) prev.textContent = content.slice(0, 60).replace(/\n/g, ' ');
    }
  }, 500);
}
...
function renderNotesList() {
  ...
  list.innerHTML = notes.map(n => `
    <div class="note-item${n.id === STATE.activeNote ? ' active' : ''}" data-note-id="${n.id}" onclick="openNote('${n.id}')">
...
function wrapText(before, after) {
  const ta = document.getElementById('note-content-inp');
  if (!ta) return;
...
function saveCloudFile() {
  ...
  const fileIcons = { folder:'📁', doc:'📄', sheet:'📊', slide:'📑', image:'🖼', pdf:'📕', other:'📎' };
  const cf = { id: uid(), service, url, name, type, desc, icon: fileIcons[type] || '📎', createdAt: Date.now() };
  STATE.cloudFiles.unshift(cf);
  save(KEYS.cloud, STATE.cloudFiles);
  renderCloud();
...
function renderCloud() {
  ...
  const serviceNames = { gdrive: 'Google Drive', mega: 'MEGA', dropbox: 'Dropbox', onedrive: 'OneDrive', other: 'Cloud' };

  grid.innerHTML = STATE.cloudFiles.map(f => `
    <div class="cloud-card">
      <div class="cloud-card-header">
        <span class="cloud-file-icon">${f.icon}</span>
        <div>
          <div class="cloud-card-title">${escHtml(f.name)}</div>
          <div class="cloud-card-service">${serviceNames[f.service] || f.service}</div>
        </div>
      </div>
...
function setTheme(theme, persist = true) {
  ...
  if (persist) {
    const prefs = loadObj(KEYS.prefs, {});
    prefs.theme = theme;
...
function setupColorSwatches() {
  ...
  const accents = [
    { name: 'purple',  color: '#7c6af7' },
    { name: 'emerald', color: '#2ec49e' },
    { name: 'rose',    color: '#f06292' },
    { name: 'amber',   color: '#f0b429' },
    { name: 'sky',     color: '#38bdf8' },
  ];
  const container = document.getElementById('accent-swatches');
  container.innerHTML = accents.map(a => `
    <div class="swatch${a.name === (prefs.accent || 'purple') ? ' active' : ''}"
         style="background:${a.color}"
         data-accent="${a.name}"
         title="${a.name}"
         onclick="setAccent('${a.name}')"></div>
  `).join('');
}
...
function exportData() {
  ...
  a.href     = url;
  a.download = `nexmark-backup-${new Date().toISOString().slice(0,10)}.json`;
  a.click();
...
function clearAllData() {
  if (!confirm('This will permanently delete all your bookmarks, notes, and cloud files. Are you sure?')) return;
  if (!confirm('Last chance — delete everything?')) return;
  [KEYS.bm, KEYS.notes, KEYS.cloud].forEach(k => localStorage.removeItem(k));
  STATE.bookmarks = []; STATE.notes = []; STATE.cloudFiles = []; STATE.activeNote = null;
  renderBookmarks(); renderNotesList(); renderCloud(); updateStats();
  document.getElementById('note-editor').innerHTML = '<div class="note-editor-placeholder"><span>← Select or create a note</span></div>';
  showToast('All data cleared', 'info');
}

// ── Google Drive OAuth (stub) ───────────────────
function connectGDrive() {
  ...
  if (!CLIENT_ID) {
    ...
    if (!id) return;
    localStorage.setItem('nx_gdrive_client_id', id.trim());
  }
  ...
  // Handle token from URL (if using implicit flow redirect)
  ...
  if (hash.includes('access_token')) {
    ...
    if (token) {
      ...
      document.getElementById('gdrive-status').textContent = '✓ Connected to Google Drive';
...
// Check on load if GDrive token in hash
